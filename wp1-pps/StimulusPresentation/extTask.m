...
for pp = 1:nTrials/nTrialsPerPage
    ...
    % Load sound
    cellG = stimStructEq.stim(:,distances(1),1,1);
    cellB = stimStructEq.stim(:,distances(2),1,1);
    cellR = stimStructEq.stim(:,distances(3),1,1);
    cellY = stimStructEq.stim(:,distances(4),1,1);
    ...
    resp(uu,1:4) = [session azi distG respG];
    resp(uu+1,1:4)=[session azi distB respB];
    resp(uu+2,1:4)=[session azi distR respR];
    resp(uu+3,1:4)=[session azi distY respY];

    save(['ext_resp_' num2str(subNum),num2str(session)],'resp');
...
results.res=resp;
save(['ext_resp_' num2str(subNum),num2str(session)],'results');
