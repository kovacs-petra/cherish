...
%
% For CherISH DC6 WP1 externalization pilot. To be called by the main 
% experimental script (extPilot.m).
% 
...
%               given subject. A column contains the raw audio.
...
%               sorted (returned) stimArray contains its sorted version 
...
% user message
disp([char(10), 'Called handleParams function with input args: ',...
...
if ~logFileFlag
    ...
    logVar(2:end, strcmp(logHeader, 'dur'))             = stimArray(:, 3);  
    logVar(2:end, strcmp(logHeader, 'distance'))        = stimArray(:, 4);
    logVar(2:end, strcmp(logHeader, 'azimuth'))         = stimArray(:, 5);
    logVar(2:end, strcmp(logHeader, 'fs'))              = stimArray(:, 7);
...
if logFileFlag
    ...
    loggedAccTrialsNo = size(logAcc, 1);
    % get the exact recorded trial and block numbers for last data point
    loggedTrialNo = cell2mat(logVar(loggedAccTrialsNo+1, strcmp(logHeader, 'trialNo'))); 
    loggednoBlocks = cell2mat(logVar(loggedAccTrialsNo+1, strcmp(logHeader, 'blockNo'))); 
