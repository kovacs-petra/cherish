...
%
% For CherISH DC6 WP1 externalization pilot. The function examines the
...
%               stim. type. Its size is "no. of unique types" X no. of identifying features, with the last 
...
% stimArray     - Stimulus array (cell), with a column storing the
%               generated audio
...
%
% NOTES:
% Hard-coded expectation for the number of columns of stimuli array!! 
%
...
disp([char(10), 'Called stim2blocks with input args: ',...
    ...
    char(10), 'blockNo: ', num2str(noBlocks)]);
...
stimTypesHdr = {'f0','distance','stimTypeIdx'};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% load stimuli
load(stimArrayFile, 'stimArray');
% repeat each stimuli in the array
stimArray = repmat(stimArray,2,1);
...
if mod(trialNo/noBlocks, 1) ~= 0
    error(['Number of trials (', num2str(trialNo), ', based on stimuli',... 
        'array) is incompatible with the number of blocks (', num2str(noBlocks), ') requested']);
end
...
%%%%%%%%%%%% HARD-CODED VALUES %%%%%%%%%%%
f0 = cell2mat(stimArray(:,2));
distance = cell2mat(stimArray(:,4));
% azimuth = cell2mat(stimArray(:,5));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% unique combinations
[stimTypes, ~, stimTypeIdx] = unique([f0,distance], 'rows');
...
if length(unique(stimTypesNumbers)) ~= 1
    disp([char(10), 'Stimulus types in terms of f0 and distance:']);
    disp(stimTypes);
    ...
    stimNoPerTypePerBlock = unique(stimTypesNumbers)/noBlocks;
...
for i = 1:size(stimTypes, 1)
    typeBlockIdx = repmat(1:noBlocks, [1, stimNoPerTypePerBlock]);  % init block indices array
...
disp([char(10), 'Detailed information about stimulus types is stored in '...
    'the stimTypes cell array']); %#ok<*CHARTEN>
