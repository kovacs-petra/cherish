...
for file = 3:length(directory)
    ...
    if strcmp(filename(1:4), "imp_")
        ...
        distance = str2double(filename(14:16));
        for i = 1:length(directory)
            Obj.Data.IR(i,1,:) = shiftdim(impl,-2); % impl is [N M], data.IR must be [M R N]
            Obj.Data.IR(i,2,:) = shiftdim(impr,-2);
            Obj.SourcePosition(i,:) = [azi ele distance]; % When the spherical coordinate system is required, the format is (azimuth elevation distance)
        end
    end
end
